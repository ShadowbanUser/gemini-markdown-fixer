...
4. **跨节点状态机 (State Machine) 修复 DOM 碎裂**：深挖发现，Gemini 官方解析器在处理相邻加粗词汇与中日韩全角标点时，会发生底层错位配对。它会错误地吞掉中间的星号并生成多余的 `<b>` 标签，将原本完整的一句话在 DOM 树上强行切碎（变成 `TextNode` + `<b>` + `TextNode` 的物理隔离状态）。由于正则表达式无法跨节点匹配，本脚本彻底弃用正则，重构为跨节点状态机。通过维护全局加粗状态 (`inBoldState`)，在遍历纯文本节点时根据 `**` 的出现动态切换状态，将物理断裂的文本无缝缝合为原生的 `<strong>` 节点，同时继续免疫 Trusted Types (CSP) 安全拦截。
